...
@WebServlet("/to/Tourguide-management/add")
public class TourGuidesAddServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            if (tourGuideFunction.isPhoneNumberExist(phoneNumber)) {
                response.sendRedirect(request.getContextPath() + "/to/Tourguide-management?error=phone_exist");
                return;
            }
            
            // Kiểm tra citizen_id đã tồn tại
            if (tourGuideFunction.isCitizenIdExist(citizenId)) {
                response.sendRedirect(request.getContextPath() + "/to/Tourguide-management?error=citizen_id_exist");
                return;
            }
            
            if (tourGuideFunction.isCardIdExist(cardId)) {
                response.sendRedirect(request.getContextPath() + "/to/Tourguide-management?error=citizen_card_id_exist");
                return;
            }
            ...
            if (isSuccess) {
                response.sendRedirect(request.getContextPath() + "/to/Tourguide-management?success=add");
            } else {
                response.sendRedirect(request.getContextPath() + "/to/Tourguide-management?error=add");
            }
