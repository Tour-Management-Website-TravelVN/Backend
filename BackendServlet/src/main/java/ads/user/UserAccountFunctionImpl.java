		...
		try {
			...
			if (rs.next()) {
				...
				if (aid == 0) {
					...
					userAccount.setAdministrator(new Administrator().builder().id(aid).build());
				}
				...
				if (cid == 0) {
					...
					userAccount.setC(new Customer().builder().id(cid).build());
				}
				...
				if (tgid == 0) {
					...
					userAccount.setTourGuide(new TourGuide().builder().id(tgid).build());
				}
				...
				if (toid == 0) {
					...
					userAccount.setTourOperator(new TourOperator().builder().id(rs.getInt("tour_operator_id")).build());
				}
	...
	public ArrayList<UserAccount> getFirst100UserAccounts() {
		...
		try {
			...
			String sql = "SELECT username, password, c_id, administrator_id, tour_guide_id, tour_operator_id, email, status "
					+ "FROM user_account " + "WHERE status <> 'DEL' " + "LIMIT 200";
			pre = this.con.prepareStatement(sql);
	...
	public boolean addUserAccount(UserAccount userAccount) {
		...
		try {
			...
			if (userAccount.getEmail() != null && !userAccount.getEmail().isEmpty()
					&& isEmailExist(userAccount.getEmail())) {
				return false;
			}
...
//	        String encodedPassword = passwordEncoder.encode(password);
	...
	public ArrayList<UserAccount> getRecentDelelteAccounts() {
		...
		try {
			...
			// Lấy 100 bản ghi
			String sql = "SELECT username, password, c_id, administrator_id, tour_guide_id, tour_operator_id, email, status "
					+ "FROM user_account " + "WHERE status = 'DEL' " + "LIMIT 100";
			pre = this.con.prepareStatement(sql);
	...
	public void getFullNameOfTO(UserAccount userAccount) {
		...
		ResultSet rs = null;
		try {
			con = this.cp.getConnection("UserAccount");
			pre = con.prepareStatement("SELECT firstname, lastname FROM tour_operator WHERE tour_operator_id = ?");
			pre.setInt(1, userAccount.getTourOperator().getId());
			rs = pre.executeQuery();
			if(rs.next()) {
				userAccount.getTourOperator().setFirstname(rs.getString(1));
				userAccount.getTourOperator().setLastname(rs.getString(2));
			}
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			...
			try {
				if (con != null)
					this.cp.releaseConnection(con, "UserAccount");
